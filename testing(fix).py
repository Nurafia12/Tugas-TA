...
batik.plot.bar(y=['Contrast', 'Dissimilarity', 'Homogeneity', 'Energy','Correlation']) #subplots=True, layout=(2,4))
plt.show()
...
def hasil_klasifikasi(data_test, data_train, k):
	for d_test in data_test:
		...
		if hasil == 0:
			...
			imgplot = plt.imshow(abc)
			plt.title('Kopi Pecah')
			plt.show()
		else :
			imgplot = plt.imshow(abc)
			plt.title('Tidak Terdefinisi')
			plt.show()
...
if __name__ == '__main__':
	hasil_klasifikasi(baca_input_csv('testing.csv'), baca_input_csv('Training.csv', kondisi=True), 9) # Nilai parameter k =9 terbagus
